...
export class ApplicationSheetConfig extends HandlebarsApplicationMixin(ApplicationV2) {
    ...
    static PARTS = {
        form: {
            ...
            template: "modules/monks-enhanced-journal/templates/application-sheet-config.hbs"
        },
        footer: {
    ...
    /** @override */
    get title() {
        return game.i18n.format("SHEETS.ConfigureTitle", { prefix: i18n(`MonksEnhancedJournal.THEME.${this.#type}`)});
    }
    ...
    async _prepareFormContext(context, options) {
        ...
        context.defaults = this.#defaults = {
            theme: {
                ...
                value: game.user.getFlag("monks-enhanced-journal", `themes.${this.#type}`, ""),
            }
        };
    }
    ...
    static async onSubmitForm(event, form, formData) {
        ...
        let oldTheme = game.user.getFlag("monks-enhanced-journal", `themes.${this.#type}`, "");

        if (theme == "") {
            game.user.unsetFlag("monks-enhanced-journal", `themes.${this.#type}`);
        } else {
            game.user.setFlag("monks-enhanced-journal", `themes.${this.#type}`, theme);
        }
